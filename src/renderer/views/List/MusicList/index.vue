<template>
  <div :class="$style.list">
    ...
    <div v-show="list.length" ref="dom_listContent" :class="$style.content">
      ...
      >
        <div
          class="list-item" :class="[{ [$style.active]: playerInfo.isPlayList && playerInfo.playIndex === index }, { selected: selectedIndex == index || rightClickSelectedIndex == index }, { active: selectedList.includes(item) }, { disabled: !assertApiSupport(item.source) }, { [$style.online]: item.source != 'local' }, { [$style.local]: item.source == 'local' }]"
        ...
        >
          ...
          <div class="list-item-cell" style="flex: 0 0 16%; padding-left: 0; padding-right: 0;">
            <material-list-buttons :index="index" :download-btn="assertApiSupport(item.source) && item.source != 'local'" :local-play-btn="hasDownloadedFile(item)" :replace-local-btn="hasDownloadedFile(item) && item.source != 'local'" @btn-click="handleListBtnClick" />
          </div>
        </div>
      </base-virtualized-list>
      <base-virtualized-list
      ...
      >
        <div
          class="list-item"
          :class="[{ [$style.active]: playerInfo.isPlayList && playerInfo.playIndex === index }, { selected: selectedIndex == index || rightClickSelectedIndex == index }, { active: selectedList.includes(item) }, { disabled: !assertApiSupport(item.source) }, { [$style.online]: item.source != 'local' }, { [$style.local]: item.source == 'local' }]"
          @click="handleListItemClick($event, index)" @contextmenu="handleListItemRightClick($event, index)"
        >
          <div class="list-item-cell no-select" :class="$style.num" style="flex: 0 0 5%;">
            <transition name="play-active">
    ...
    const handleListBtnClick = ({ action, index }) => {
      switch (action) {
        ...
        case 'localPlay':
          ...
          break
        case 'replaceLocal':
          void handleReplaceWithLocalFile(index)
          break
      }
    }
